...
# Function 1 - for individual-level datasets

TB_Census_cleaning_fn_individual <- function(data_file_base, data_item_name, calendar_year,code_or_name,census_tag, census_filter_col_name, claire_redownload = F){
  ...
  for(i in 1:6){
    ...
    #2. find geo column 
    if(code_or_name == "name" & geog_list[i]!="national" & geog_list[i]!="National"){ #skip national - no code
...
# Function 2 - for dwelling-level or family-level datasets




TB_Census_cleaning_fn_dwelling <- function(data_file_base, data_item_name, calendar_year,code_or_name,census_tag, census_filter_col_name, claire_redownload = F){
