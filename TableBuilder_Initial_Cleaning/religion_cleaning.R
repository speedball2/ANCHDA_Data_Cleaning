...
TB_Census_cleaning_fn <- function(data_file_base, data_item_name, calendar_year,code_or_name){
  ...
  for(i in 1:length(geog_list)){ #loop over the 6 geography types
    ...
    colnames(data_temp)[5] <- data_item_name
    
    
    # delete trailing crap rows
    trailing_rows <- which(is.na(data_temp[,5]))
...
TB_Census_cleaning_fn(data_file_base = "census_year12", data_item_name ="Completed_Year12", calendar_year=2011,code_or_name = "code")
