...
setwd("C:/Users/00095998/OneDrive - The University of Western Australia/acwa_temp/abs_income/") #change this path if you copy this 
...
for (file in csv_files) {
  ...
  # Check if the n_live_in_household_income_less_1000 column exists in the data frame
  if ("n_live_in_household_income_less_1000" %in% colnames(df)) {
    # Apply cell suppression
    df[df[,"n_live_in_household_income_less_1000"] <5 & !is.na(df[,"n_live_in_household_income_less_1000"]), -c(1:4)] <- 9999999
  } else {
    # Print a message to indicate that the column was not found
    cat("Skipping file", file, "because it does not contain the n_live_in_household_income_less_1000 column.\n")
  }
