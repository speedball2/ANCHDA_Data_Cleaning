---
title: "format check and cell suppression on NSSHS data"
...
for (file in csv_files) {
  ...
  suppressed_cols <- 5  # Column position for cell suppression (5th column)
  suppressed_rows <- sum(df[, suppressed_cols] > 0 & df[, suppressed_cols] < 5 & !is.na(df[, 6]))
  df[df[, suppressed_cols] > 0 & df[, suppressed_cols] < 5 & !is.na(df[, 6]), -c(1:4)] <- 9999999
  
  # Print the count of suppressed rows
  cat("Number of rows suppressed in", file, ":", suppressed_rows, "\n")
