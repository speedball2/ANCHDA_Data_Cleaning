...
setwd("C:/Users/00095998/OneDrive - The University of Western Australia/acwa_temp/air/") #change this path if you copy this 
...
Define the output directory path to save cleaned dataset WITH CELL SUPPRESSED # YOU DONT NEED TO CREATE THE SUBFOLDER R DOES IT FOR YOU
```{r}
...
for (file in csv_files) {
  ...
  # Apply cell suppression to 'uncertainty' column #NOT NECESSARY HERE BUT AS AN EXAMPLE
  if ('uncertainty' %in% colnames(df)){
    df[df[, "uncertainty"] == 2 & !is.na(df[, "uncertainty"]), colnames(df) %in% 'uncertainty' | colnames(df) %in% names(df)[1:5]] <- 9999999
  }
  
  # Remove the 'uncertainty' column from the data frame #NOT NECESSARY HERE BUT AS AN EXAMPLE
