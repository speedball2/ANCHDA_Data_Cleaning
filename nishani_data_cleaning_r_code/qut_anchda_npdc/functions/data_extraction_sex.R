...
data_extraction_sex <- function(data, colnumbers, n_name, p_name, file_name_base){
  ...
  new_data[,4:5]<- lapply( new_data[,4:5], function(x)gsub("n.p.", "NULL", x))
  ...
  SA4_new_data_p$p <- unlist(rounding_fun(SA4_new_data_p$p))
  ...
  STE_new_data_p$p <- unlist(rounding_fun(STE_new_data_p$p))
