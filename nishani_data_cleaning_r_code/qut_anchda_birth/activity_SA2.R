...
table <- c("Table 2.1", "Table 2.2", "Table 2.3", "Table 2.4", "Table 2.5", "Table 2.6", "Table 2.7", "Table 2.8" )
data_range <- c("L8:M775", "P8:Q775", "T8:U775", "X8:Y775","AB8:AC775","AF8:AG775","AJ8:AK775","AN8:AO775", "AR8:AS775")
year <- 2013:2021
...
for(j in 1:length(table)){
  for(i in 1:length(data_range)){
    ...
    SA2_CODE21 <- read_files("./data/births_SA2_2011-2021.xlsx", table[j] , "A8:A775")    
    names(SA2_CODE21) <- "SA2_CODE21"
    data_3_1  <- read_files("./data/births_SA2_2011-2021.xlsx", table[j] , data_range[i])
    names(data_3_1) <- c("n_births","fertility_rate")
    data_3_1$calendar_year <- year[i]
    data_3_1$sex <- "all"
    data_3_1$age_group <- "0-0"
    data_3_1 <- cbind(SA2_CODE21,data_3_1)
    
    if(length(which(nchar(data_3_1$SA2_CODE21) < 9)) > 0){
      
      data_3_1 <- data_3_1[-which(nchar(data_3_1$SA2_CODE21) < 9),]
    }
...
#----------------------------------
data_3_1_full_birth <- data_3_1_full[, c("SA2_CODE21","calendar_year", "sex", "age_group", "n_births")]
...
#------------------------------
data_3_1_full_fertility <- data_3_1_full[, c("SA2_CODE21","calendar_year", "sex", "age_group", "fertility_rate")]
...
write.csv(data_3_1_full_birth, "./output/ABS_Births_1110_n_birth_SA2.csv", row.names = FALSE)

write.csv(data_3_1_full_fertility, "./output/ABS_Births_1114_fertility_rate_SA2.csv", row.names = FALSE)
#------------------------
combined_data <- left_join(data_3_1_full_birth,data_3_1_full_fertility, by = c("SA2_CODE21", "calendar_year", "sex","age_group")) 

write.csv(combined_data, "./output/ABS_Births_1110_1114_n_birth_fertility_rate_SA2.csv", row.names = FALSE)