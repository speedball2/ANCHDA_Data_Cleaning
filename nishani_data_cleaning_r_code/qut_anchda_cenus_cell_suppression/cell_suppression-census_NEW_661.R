...
library(dplyr)
#split SA2 files by year to run the cell suppression
...
for(m in 1:length(u_year)){
  ...
  write.csv(sub_data,paste0("C:/Users/Nishani/Documents/QUT_ANCHDA/qut_anchda_cenus_cell_suppression/data/census/proportion_calculation/661/", base_file_name, "_", u_year[m], "_", geo_name, ".csv"), row.names = FALSE)
  
}

#---------------------------------------

base_file_name <- "ABS_census_661_country_of_birth_2006"
...
for(i in 1:length(geo_name)){
  ...
  data <- read.csv(paste0("C:/Users/Nishani/Documents/QUT_ANCHDA/qut_anchda_cenus_cell_suppression/data/census/proportion_calculation/661/" ,base_file_name, "_", geo_name[i], ".csv" ), header = TRUE, check.names = FALSE)
...
cell_suppression <- function(data,  count_indicator_name, rate_indicator_name, name_uncertiniy_col, filetrs, base_file_name, geo){
  ...
  u_fil <- unique(data[, "region_children_and_young_people_born"])
  ...
  for(i in 1:length(u_code)){
    ...
    for(j in 1:length(u_age)){
      ...
      for(k in 1:length(u_fil)){
        ...
        sub_data <- data[which(data[,SA_CODE]== u_code[i] & data[,"age_group"]== u_age[j] & data[, "region_children_and_young_people_born"] == u_fil[k]),]
        pp <- pp + 1
  ...
  write.csv(full_data, paste0("C:/Users/Nishani/Documents/QUT_ANCHDA/qut_anchda_cenus_cell_suppression/data/census/cell_suppressed/661/" ,base_file_name, "_", geo, ".csv" ) , row.names = FALSE)
...
files_632 <- list.files(path = "C:/Users/Nishani/Documents/QUT_ANCHDA/qut_anchda_cenus_cell_suppression/data/census/cell_suppressed/661/", full.names = TRUE)
...
write.csv(new_data, "C:/Users/Nishani/Documents/QUT_ANCHDA/qut_anchda_cenus_cell_suppression/data/census/cell_suppressed/661/ABS_census_661_country_of_birth_SA2.csv", row.names = FALSE )
