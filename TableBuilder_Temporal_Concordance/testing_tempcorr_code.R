...
# - pivot_wider any additional filter vars from tablebuilder

# Rename columns to GEO_CODE_YEAR format that matches correspondence df column names
...
df <- read.csv("/Users/Current/OneDrive - Queensland University of Technology/General - ACWA_QUT/Data_Collections_INTERIM/Census_Interim_Pre-Temporal-Concordance/census_core_activity_need_assistance_LGA_2021_INTERIM.csv")

names(df)

# IF LGA - strip "LGA" string off front
df$LGA..UR. <- stringr::str_sub(df$LGA..UR.,4,-1)

names(df)[1] <- "LGA_CODE_2021"

# IF 2021 -- need to take inverse of from:to ratio


# Convert this to a function: ------------------------------------------------ #
# User inputs:
GEO_FROM <- "LGA_CODE_2021"                                                     # Original data set concordance year.
GEO_TO <- "LGA_CODE_2016"                                                       # Target year.
GEO_NAME <- "LGA_CODE16"                                                        # Desired name for geography code.
VAR_NAME <- "core_activity_need_for_assistance"                                                               # Name of the input variable.
FILTER_VARS <- c("AGE5P.Age.in.Five.Year.Groups", "SEXP.Sex", "ASSNP.Core.Activity.Need.for.Assistance")                                                # Name of original data set filter variable(s).
...
for (i in 1:nrow(filter_list)){
  ...
  for (k in 1:length(names(filter_list))){
    df.filt <- df.filt %>% filter(df[,which(names(df) %in% names(filter_list)[k])] == filter_list[i,k]) #this is busted
  }
  for (j in 1:nrow(df.new)){
    df.new[j,2] <- round(sum(df.filt$val[which(df.filt$LGA_CODE_2016 %in% df.cor$LGA_CODE_2016[ls.cor[[j]]])] * 1/df.cor$RATIO_FROM_TO[ls.cor[[j]]]),2)
  }
